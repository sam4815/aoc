...
(defn sum-object
  ([string omit-red]
   (sum-object string omit-red 1))
  ([string omit-red start-index]
   (loop [n start-index sum 0 has-red false]
     (case (get string n)
       \{ (let [[object-sum last-n] (sum-object string omit-red (inc n))]
...
(println (format "The sum of all numbers in the document is %d." (first (sum-object json false))))
(println (format "Omitting red objects, the sum of all numbers in the document is %d." (first (sum-object json true))))
